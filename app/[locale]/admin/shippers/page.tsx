...
export default async function AdminShippersPage() {
  ...
  if (!token) redirect(`/${locale}/login?next=/${locale}/admin/shippers`);

  let session;
  try {
    session = await verifySessionToken(token);
  } catch {
    redirect(`/${locale}/login?next=/${locale}/admin/shippers`);
  }
  ...
  if (!user) redirect(`/${locale}/login?next=/${locale}/admin/shippers`);
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ marginTop: '0.5rem' }}>{locale === 'ar' ? 'إدارة عروض السوّاق' : 'Manage shippers offers'}</p>
